...
FROM
    salaries AS s
        INNER JOIN
    dept_emp ON s.emp_no = dept_emp.emp_no
        INNER JOIN
...
SELECT 
    ...
    SUM(salary) AS money_received_by_department
FROM
    dept_emp AS d_e
        INNER JOIN
    departments AS d ON d_e.dept_no = d.dept_no
        INNER JOIN
    salaries AS s ON s.emp_no = d_e.emp_no
WHERE
    s.to_date = '9999-01-01'
GROUP BY d.dept_name
